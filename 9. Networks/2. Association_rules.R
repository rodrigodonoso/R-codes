...
playlists <- split(x=lastfm$artist, f=lastfm$user)  #manera de organizar el data 
...
musicrules <- apriori(playtrans, 
                      parameter=list(support=.01, confidence=.5, maxlen=3)) #puso altos % de support y confidence.

#EXAMPLE OF CHIPS: suppose chips are purchased 10% of the time in general but 50% of the time when consumer grab beer.
#SUPPORT: P(AyB) = 10%
#CONFIDENCE: P(B/A) = 50%
#LIFT: P(B/A)/P(B) = 5, 5 times higher than 10%.
...
pairs <- gsub("\\{|\\}","",pairs) #funcion para manipular strings, remove brackets
pairs <- strsplit(pairs," => ") #split con arroww
pairs <- do.call(rbind,pairs) #ponerlo todo en una gran matrix
...
which.max(mbetween) #cual tiene mas betweenness: the beatles


## and raw popularity
playcount <- table(lastfm$artist)[names(mdegree)]
which.max(playcount) #cual es el mas popular
...
points(playcount["pid??ama porno"],mbetween["pid??ama porno"],col=2)
text(playcount["pid??ama porno"],
     mbetween["pid??ama porno"]*2,labels="pid??ama porno",col=2) 
...
band <- "rush" 
nei <- graph.neighborhood(musicnet, 1, V(musicnet)[band])[[1]] #1 (al lado de musicnet es cuan lejos estoy)
